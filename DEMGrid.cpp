...
long DEMGrid::read(std::istream& dem, bool incrementalRead)
{
  if (_firstTime)
  {
    dem >> _header;
...
DEMHeader const& DEMGrid::getHeader() const
{
  return _header;
}

long DEMGrid::getWidth() const
{
  return _width;
}

long DEMGrid::getHeight() const
{
  return _height;
}

float DEMGrid::getElevation(long x, long y) const
{
  if (_grid == 0)
    return _missDataVal;
...
float DEMGrid::getMissingDataValue() const
{
  return _missDataVal;
}

long DEMGrid::fillGeographic(std::istream& dem,bool incrementalRead)
{
  if (_firstTime)
  {
    _curProfile = 0;
...
long DEMGrid::fillUTM(std::istream& dem, bool incrementalRead)
{
  ...
  // Now, insert the elevations in the profiles in the appropriate place in
  // the grid.
...
void DEMGrid::getGroundCoords(long x, long y, 
			      double& ground_x, double& ground_y)
{
